...
- Complete API mock coverage for every route, JSON/binary/no-content success response, and documented status, including
  backup blob `200`, generation `500`, and delete `204`.
...
- Add isolated browser-level journeys for connection setup, manual add, ISBN lookup/edit, checkout/check-in, reading,
  delete/restore, authenticated SQL backup, and updated dashboard values.
...
- Backup coverage verifies a non-empty SQL attachment, filename handling, object-URL cleanup, generation `500`, and no
  download after failure. It includes missing/malformed filenames, interrupted download, production-like
  preflight/header exposure, and a safe fallback filename.
- Tests never parse, log, cache, snapshot, upload, or otherwise expose SQL backup contents.
