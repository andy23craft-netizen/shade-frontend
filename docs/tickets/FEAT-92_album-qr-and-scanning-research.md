...
**Status:** Pending review with physical labels 
...
- [x] Album labels encode `shade:v1:album:<album_id>` with no tenant name or public URL.
- [x] Reprinting preserves the same album UUID and uses the shared single/batch print
      templates and validation rules.
- [x] A valid album Shade payload resolves to zero or one typed owned-copy summary containing
      title, primary artist, format, state, crate, checkout eligibility, and active-loan
      context.
- [x] Malformed or unsupported Shade payloads and well-formed unknown/other-tenant labels use
      the shared safe error behavior.
- [x] Commercial album barcodes may return multiple owned copies and present explicit copy
      selection rather than guessing.
- [x] Exact-copy scans offer the appropriate checkout/check-in action based on lifecycle
      state and active-loan context.
- [x] When Enable Loans is off, scanning can still resolve/open an album but exposes no new
      circulation action.
