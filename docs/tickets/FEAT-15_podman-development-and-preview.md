...
- **Copy or bind-mount** the repo-root `.env` into the container at startup so Vite can read `VITE_API_SECRET_KEY`
  during development and preview workflows. Operators maintain a local gitignored `.env`; prefer bind-mount over baking
  `.env` into image layers when feasible. Do not log the token value.
- Serve client routes with an SPA fallback and appropriate preview cache behavior (every registered product path,
  including `/shelves`, `/admin/deleted`, and `/admin/backup`).
...
- The container has access to the repo-root `.env` file (copied or bind-mounted at startup) for
  ...
  the token value. Built JS may contain the build-time secret (same as non-container production builds).
