...
refetch/messaging). Book details already gates "Check Out" to active `available` books (`canCheckout`: not deleted
and `status === 'available'`).

FEAT-21 display-only alternate-copy UX is complete on `/checkout` (`displayOnlyAlternatives`, `checkoutEligibility`).
This ticket removes that page; do not re-home a substitute chooser onto book details. If `displayOnlyAlternatives` is
only imported by `CheckoutPage`, delete it with the page.

FEAT-22 check-in consolidation is complete (ticket file removed). Product check-in is `CheckinForm` on `/loans`;
`/checkin` is `LegacyCheckinRedirect` (replace-navigate to `/loans`, forward search). Circulation drawer is Check Out
and Loans only. Reuse that redirect pattern for `/checkout`; do not revert check-in onto a dedicated page.

FEAT-18 collection filters and FEAT-19 wishlists are complete; do not reference their removed ticket files.

This ticket may ship in its own PR. Do not block on or bundle FEAT-24 (scanner on more pages).

Do not pull FEAT-15 Podman, FEAT-16 release artifacts, FEAT-17 About / homepage, or FEAT-20 dashboard reports into
this implementation beyond retargeting workflow links away from `/checkout`.
...
- `/checkout` via `CheckoutPage` + `checkoutModel` + `useCheckoutBook`. Eligible-book picker from `useBooks` filtered
  by `isCheckoutEligible`. Optional ISBN Find via checksum-gated `useBooks({ isbn })` with typed / camera / hardware
  handoff (lazy `IsbnCameraScanner`). `?bookId=` deep-link. Form fields: Book, Borrower, Checkout date and time
  (`datetime-local`, blank → omit so the API uses UTC now), Due date (`date`), Notes. `ConfirmationDialog` before
  mutate. Field-linked **422**. **404** / **409** / **412** stale refetch with preserved form input. After a
  display-only block (mutate **412** or selected/deep-linked `display_only`), `displayOnlyAlternatives` offers eligible
  substitutes via `isbn` and `author`+`title` list filters. Success navigates to `/books/{id}`.
- `checkoutFormValuesToRequest` omits blank optionals and normalizes a supplied checkout timestamp to UTC ISO 8601;
  due date stays date-only. Values still include `checked_out_at` / `due_at` (operator-editable).
- `checkoutEligibility.ts`: `isCheckoutEligible` (`deletion_date === null` and `status === 'available'`). Used by
  `CheckoutPage` and `displayOnlyAlternatives`. No colocated `checkoutEligibility.test.ts` yet. `BookDetailsPage` still
  inlines the same predicate as `canCheckout` instead of importing the helper.
- Detail "Check Out" on `BookDetailsPage` is an `AppLink` to `/checkout?bookId=...` when `canCheckout`. Display-only
  and on-loan books do not get the link. Detail "Check In" links to `/loans?bookId=...` when `isCheckinEligible`.
...
- Primary nav in `AppShell`: direct Dashboard link; Collection `DrawerNavMenu` (Browse, Manage, Wishlists);
  Circulation `DrawerNavMenu` with Check Out and Loans (no Check In item). Check Out is a drawer item, not a flat
  header link.
- `LoansPage` empty state links "Check Out a Book" to `/checkout`. Product check-in is `CheckinForm` on `/loans`.
- `/checkin` is `LegacyCheckinRedirect` (path-only `routeMetadata.checkin`; no "Check In" document title).
...
mainly to pick a book, type timestamps, Find-by-ISBN, and offer display-only alternates -- none of which are needed
once checkout lives on details.
...
6. **Honest stale outcomes** -- **409** `Book is already checked out`, **412** `Book is display only`, and **404**
   ...
   "Use Find by ISBN" or to pick an alternate copy. In-flight submit is disabled. Eligibility is still
   `deletion_date === null` and `status === 'available'`; do not offer Check Out for `display_only`.
7. **Old `/checkout` URLs** -- replace-navigate `/checkout` to `/books`, and `/checkout?bookId={id}` to
   ...
   "Check Out" document title (mirror `LegacyCheckinRedirect` / path-only `routeMetadata.checkin`).
...
- Do not stack a second `ConfirmationDialog` on top of the form dialog. The popup plus named submit is the
  ...
  `ConfirmationDialog`'s cancel-first confirm pattern. (`CheckinForm` still uses a nested confirm; do not copy that
  for checkout.)
...
- Re-implementing FEAT-21 alternate-copy offers on details (no substitute chooser).
...
| `src/features/loans/checkoutEligibility.ts` | Keep. Import from `BookDetailsPage` and `CheckoutDialog` so gating cannot drift (stop inlining `canCheckout`). Add colocated `checkoutEligibility.test.ts` if still missing. |
...
| File | Change |
| ---- | ------ |
| `src/routes/routeMetadata.ts` | Make `checkout` path-only like `checkin` (no title/heading). Book details title/heading unchanged. |
| `src/routes/routes.tsx` | Stop importing `CheckoutPage`. Add a compatibility child at path `/checkout` that replace-navigates to `/books` or, when `bookId` is in the search string, to `/books/{bookId}?checkout=1` (forward only `bookId`, as `checkout=1`). Do not give that redirect a "Check Out" `handle.title`. Mirror `LegacyCheckinRedirect`. |
| `src/routes/LegacyCheckoutRedirect.tsx` (new, optional) | Tiny redirect next to `LegacyCheckinRedirect` if inline `Navigate` in `routes.tsx` is awkward; keep it out of titled `routeMetadata`. |
...
| `src/features/loans/displayOnlyAlternatives.ts` / `.test.ts` | Delete if unused after `CheckoutPage` is gone. Do not wire alternates into `CheckoutDialog`. |
| `src/layout/AppShell.tsx` | Remove the Check Out item from the Circulation `DrawerNavMenu` items and drop `/checkout` from `activePrefixes`. Remaining Circulation item is Loans. |
| `src/layout/AppShell.test.tsx` | Open the Circulation drawer and assert Check Out is absent (Check In stays absent). Change the client-nav smoke that currently opens the Circulation drawer and clicks Check Out (expecting heading "Check Out Book") to use Loans. Assert there is no drawer or header link to `/checkout`. Cover `/checkout` → `/books` (and `?bookId=` → details `?checkout=1`) the same way `/checkin` → `/loans` is covered. |
...
| `e2e/library.lifecycle.spec.ts` | From book details, Check Out is a button that opens a dialog (heading in the dialog, not a page `h1` "Check Out Book"). Fill Borrower, press "Check Out Book", assert `POST /books/{id}/checkout` via `mockApi` (no nested "Confirm checkout"). Stay on the same book heading after success; status `on_loan`. Then continue the existing check-in on `/loans` / mark-read journey. |
...
Workstream 6 already shipped checkout as a dedicated page with optional timestamps, ISBN Find, and FEAT-21
display-only alternates. This ticket is IA cleanup plus a smaller form: one dialog on details, same lifecycle
endpoint, computed now and 366-day due date. Explicitly excludes re-homing alternate copies and FEAT-15 through
FEAT-20 product work except doc/nav mentions of `/checkout`. FEAT-22 is already shipped; do not bundle FEAT-24.
