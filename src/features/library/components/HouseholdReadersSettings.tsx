    ...
    return <section className="library-settings-form__household" aria-labelledby="household-readers-heading">
        ...
        <ul className="library-settings-form__profile-list" aria-label="Household readers">
            {profiles.map((profile) => <li key={profile.profile_id}>
                {editing === profile.profile_id ? <div className="library-settings-form__profile-edit"><Field label={profile.is_owner ? 'Owner name' : 'Reader name'} error={editError}><input value={editName} maxLength={255} onChange={(event) => setEditName(event.target.value)} /></Field><div className="button-row"><Button type="button" disabled={update.isPending} onClick={() => saveEdit(profile)}>{update.isPending ? 'Saving…' : 'Save name'}</Button><Button type="button" variant="secondary" disabled={update.isPending} onClick={() => { setEditing(null); setEditError(null); update.reset() }}>Cancel</Button></div></div> : <><span><strong>{profile.display_name}</strong>{profile.is_owner ? ' (Owner)' : ''}</span><div className="button-row"><Button type="button" variant="secondary" disabled={update.isPending || remove.isPending} onClick={() => { setEditing(profile.profile_id); setEditName(profile.display_name); setEditError(null); update.reset() }}>Rename</Button>{!profile.is_owner ? <Button type="button" variant="danger" disabled={update.isPending || remove.isPending} onClick={() => openRemoval(profile)}>Remove</Button> : null}</div></>}
            </li>)}
        </ul>
        <div className="library-settings-form__add-reader">
            ...
            <Button type="button" onClick={add} disabled={create.isPending}>{create.isPending ? 'Adding…' : 'Add reader'}</Button>
        </div>
        ...
        <ConfirmationDialog open={removing !== null} title="Remove household reader?" confirmLabel={outcome === 'delete' ? 'Delete personal records' : 'Reassign records and remove'} confirmVariant="danger" confirmDisabled={remove.isPending || (outcome === 'reassign' && !targetProfileId)} onCancel={() => { if (!remove.isPending) setRemoving(null) }} onConfirm={confirmRemoval}>
