...
export function AddAlbumToCollectionControl({ albumId, albumTitle }: { albumId: string; albumTitle: string }) {
    ...
    return <form className="add-album-to-collection" onSubmit={submit}>{message ? <Alert variant={message.variant}>{message.text}</Alert> : null}<Field label="Collection"><select value={collectionId} onChange={(event) => setCollectionId(event.target.value)} disabled={collections.isPending || add.isPending}><option value="">Choose a collection</option>{eligibleCollections.map((collection) => <option key={collection.collection_id} value={collection.collection_id}>{collection.name}</option>)}</select></Field><Field label="Collection description"><textarea rows={3} value={notes} onChange={(event) => setNotes(event.target.value)} /></Field><Button type="submit" mutating disabled={!collectionId || add.isPending}>{add.isPending ? 'Adding…' : 'Add to Collection'}</Button></form>
}
