...
export function ListeningDashboardPage() {
    ...
    return <section className="route-page listening-dashboard">
        ...
        {dashboard.reader_analytics && dashboard.reader_analytics.length > 1 ? <section className="household-analytics" aria-labelledby="household-listening-analytics"><h2 id="household-listening-analytics">Household listening analytics</h2><p>Personal listening records are shown separately for each household reader.</p><div className="household-analytics__grid">{dashboard.reader_analytics.map((reader) => <section key={reader.profile.profile_id} aria-labelledby={`listener-${reader.profile.profile_id}`}><h3 id={`listener-${reader.profile.profile_id}`}>{reader.profile.display_name}</h3><dl className="dashboard-metrics"><div><dt>Albums played</dt><dd>{reader.listening.albums_played}</dd></div><div><dt>Unplayed</dt><dd>{reader.listening.albums_unplayed}</dd></div><div><dt>Average rating</dt><dd>{average(reader.listening.average_rating, ' / 5')}</dd></div></dl></section>)}</div></section> : null}
    </section>
}
