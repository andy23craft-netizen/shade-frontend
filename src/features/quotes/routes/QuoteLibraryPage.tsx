...
export function QuoteLibraryPage() {
    ...
    return <section className="route-page quote-library-page">
        ...
        <form className="quote-library-form" onSubmit={(event) => void save(event)}>
            ...
            <div className="button-row"><Button type="submit" mutating disabled={pending || !draft.text.trim() || !draft.author.trim()}>{editing ? 'Save Quote' : 'Add Quote'}</Button>{editing ? <Button type="button" variant="secondary" onClick={() => { setEditing(null); setDraft(emptyDraft) }}>Cancel</Button> : null}</div>
        </form>
        <section aria-labelledby="saved-quotes"><h2 id="saved-quotes">Saved quotes</h2>
            ...
            <ol className="quote-library-list">{items.map((quote, index) => <li key={quote.quote_id}>
                ...
                <div className="button-row"><Button type="button" variant="secondary" mutating disabled={pending || index === 0} onClick={() => void move(index, -1)}>Move Up</Button><Button type="button" variant="secondary" mutating disabled={pending || index === items.length - 1} onClick={() => void move(index, 1)}>Move Down</Button><Button type="button" variant="secondary" disabled={pending} onClick={() => startEdit(quote)}>Edit</Button><Button type="button" variant="secondary" mutating disabled={pending} onClick={() => void update.mutateAsync({ quoteId: quote.quote_id, quote: { enabled: !quote.enabled } })}>{quote.enabled ? 'Disable' : 'Enable'}</Button><Button type="button" variant="danger" mutating disabled={pending} onClick={() => setConfirm({ kind: 'delete', quote })}>Delete</Button></div>
            </li>)}</ol>
        </section>
        <section><h2>Restore defaults</h2><p>This replaces every custom quote with the library defaults.</p><Button type="button" variant="danger" mutating disabled={pending} onClick={() => setConfirm({ kind: 'restore' })}>Restore Default Quotes</Button></section>
        <ConfirmationDialog open={confirm !== null} title={confirm?.kind === 'restore' ? 'Restore default quotes?' : 'Delete this quote?'} confirmLabel={confirm?.kind === 'restore' ? 'Replace Custom Quotes' : 'Delete Quote'} confirmVariant="danger" mutating onCancel={() => setConfirm(null)} onConfirm={() => { const current = confirm; setConfirm(null); if (current?.kind === 'restore') void restore.mutateAsync().then(() => setMessage('Default quotes restored.')); else if (current) void remove.mutateAsync(current.quote.quote_id).then(() => setMessage('Quote deleted.')) }}><p>{confirm?.kind === 'restore' ? 'All custom quotes will be permanently replaced by the default set.' : 'This quote will be permanently removed.'}</p></ConfirmationDialog>
    </section>
}
