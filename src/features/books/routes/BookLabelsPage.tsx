...
function Label({ bookId, title }: { bookId: string; title: string }) {
    ...
    return <article className="book-label">
        {image ? <img src={image} alt={`Shade label for ${title}`} /> : <span>Generating code…</span>}
        <strong>{title}</strong>
...
export function BookLabelsPage() {
    ...
    const position = Number.isInteger(start) && start >= 1 && start <= 6 ? start : 1
    ...
    return <section className="route-page book-labels-page">
        ...
        {books.length === 0 ? <Alert variant="warning">{all ? 'There are no catalog books to label.' : 'Select one or more books from Browse, then choose Print labels for selected books.'}</Alert> : <>
            <div className="book-labels-page__controls no-print">
                <label>Start at sheet position <select value={position} onChange={(event) => { params.set('start', event.target.value); setParams(params) }}>
                    {[1, 2, 3, 4, 5, 6].map((value) => <option key={value} value={value}>{value}</option>)}
                </select></label>
                <Button type="button" onClick={() => window.print()}>Print labels</Button>
                <AppLink to="/books" variant="secondary">Back to Books</AppLink>
            </div>
            <p className="no-print" role="status">{all && booksQuery.hasNextPage ? `Loading more catalog books; ${books.length} ready so far.` : `${books.length} ${all ? 'catalog' : 'selected'} ${books.length === 1 ? 'book' : 'books'} ready. Labels print two across by three down on US Letter.`}</p>
