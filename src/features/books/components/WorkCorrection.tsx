...
export function WorkCorrection({ book }: { book: BookRead }) {
    ...
    return <section className="book-details-panel book-work-correction"><h2>Possible duplicate copies</h2><p>Only ISBN or title-and-author matches are offered. Check title, author, and shelf against the physical copy.</p>
        ...
        <Button variant="secondary" mutating disabled={!candidate || busy} onClick={() => setAction('merge')}>Group as Same Work</Button>
        {copies.length > 1 ? <Button variant="secondary" mutating disabled={busy} onClick={() => setAction('split')}>Separate from Work</Button> : null}
