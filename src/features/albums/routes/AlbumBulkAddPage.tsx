...
import { AlbumVocabularyPicker } from '../components/AlbumForm'
...
export function AlbumBulkAddPage() {
        ...
        void lookup.mutateAsync({ items: pendingLookup.map(item => item.kind === 'manual'
            ...
            .then(response => setQueue(current => current.map(item => {
                ...
                return { ...item, result, status: result.status, draft: { ...hydrated,
                    artistIds: hydrated.artistNames.map(name => artists.data?.items.find(a => normalizeName(displayArtist(a)) === normalizeName(name))?.person_id).filter((id): id is string => Boolean(id)),
    ...
    const addScannedBarcode = (value: string) => {
        setScannerOpen(false); setKind('barcode'); setError(null)
        if (queue.length >= MAX_ITEMS) { setError(`A session can contain at most ${MAX_ITEMS} albums.`); return }
        const clientItemId = `album-${Date.now()}-${sequence.current++}`
        setQueue(current => [...current, { clientItemId, kind: 'barcode', value, status: 'queued', draft: emptyAlbumDraft() }])
        setMessage('1 album added to the queue.')
    }
    ...
    const removeFromQueue = (id: string) => setQueue(current => current.filter(item => item.clientItemId !== id))
    ...
    const addCanonicalArtist = async (id: string) => { const raw = window.prompt('Artist name')?.trim(); if (!raw) return; const parsed = parseManualArtists(raw)[0]; const created = await createArtist.mutateAsync(parsed); updateDraft(id, { artistIds: [...(queue.find(x => x.clientItemId === id)?.draft.artistIds ?? []), created.person_id] }) }
    ...
    return <section className="route-page bulk-add-page album-bulk-add-page" aria-labelledby="album-bulk-title">
        ...
        {!started ? <form className="bulk-add-setup" onSubmit={event => { event.preventDefault(); if (shelfName) setStarted(true) }}><h2>Choose a destination crate</h2><Field label="Destination crate"><select value={shelfName} onChange={event => setShelfName(event.target.value)} required><option value="">Select a crate</option>{assignableShelves.map(shelf => <option key={shelf.shelf_id} value={shelf.common_name}>{formatShelfCommonNameForDisplay(shelf.common_name)}</option>)}</select></Field><div className="form-actions"><Button type="submit" disabled={!shelfName}>Start capture</Button><AppLink className="button button--secondary" to="/shelves">Manage crates</AppLink></div></form> : <>
            ...
            <form className="bulk-add-capture" onSubmit={addCapture}><div><h2>Capture releases</h2><p>Barcode and Discogs inputs accept comma- or line-separated batches.</p></div><div className="bulk-add-capture__form"><Field label="Capture type"><select value={kind} onChange={event => { const next = event.target.value as AlbumCaptureKind; setKind(next); if (next !== 'barcode') setScannerOpen(false) }}><option value="barcode">Barcode</option><option value="discogs">Discogs release ID</option><option value="manual">Manual artist/title</option></select></Field>{kind === 'manual' ? <><Field label="Title"><input value={manualTitle} onChange={event => setManualTitle(event.target.value)} /></Field><Field label="Artists" helpText="Separate artists with semicolons"><input value={manualArtists} onChange={event => setManualArtists(event.target.value)} /></Field></> : <Field label={kind === 'barcode' ? 'Barcodes' : 'Discogs release IDs'}><textarea rows={3} value={capture} onChange={event => setCapture(event.target.value)} /></Field>}{kind === 'barcode' ? <Button type="button" variant="secondary" disabled={queue.length >= MAX_ITEMS} onClick={() => setScannerOpen(true)}>Scan barcode</Button> : null}<AppLink to="/catalog/image-search" variant="secondary">Search by image</AppLink><Button type="submit" mutating disabled={queue.length >= MAX_ITEMS}>Add to queue</Button></div></form>
            {scannerOpen ? <Suspense fallback={<LoadingState label="Loading camera scanner…" />}><AlbumBarcodeCameraScanner onDetected={addScannedBarcode} onCancel={() => setScannerOpen(false)} /></Suspense> : null}
            {message ? <Alert variant="info">{message}</Alert> : null}{error ? <Alert variant="error">{error}</Alert> : null}
            <section className="bulk-add-queue"><div className="bulk-add-queue__heading"><div><h2>Review queue</h2><p>{queue.length} of {MAX_ITEMS} rows</p></div></div>{queue.length === 0 ? <p className="bulk-add-queue__empty">Capture a release to begin.</p> : <ol className="bulk-add-queue__list">{queue.map(item => <li className="bulk-add-queue-item" key={item.clientItemId}><div className="bulk-add-queue-item__main"><div><strong className="bulk-add-queue-item__title">{item.draft.title || item.value || 'Manual album'}</strong><span className="bulk-add-queue-item__isbn">{item.kind === 'manual' ? 'Manual entry' : `${item.kind === 'barcode' ? 'Barcode' : 'Discogs'}: ${item.value}`}</span></div></div><div className="bulk-add-queue-item__actions"><div className="bulk-add-queue-item__status"><span>{item.saveStatus ? item.saveStatus.replaceAll('_', ' ') : statusDetail(item)}</span></div>{item.saveStatus !== 'created' && item.saveStatus !== 'wishlist_acquired' ? <button type="button" className="bulk-add-queue-item__remove" aria-label={`Remove ${item.draft.title || item.value || 'album'} from queue`} onClick={() => removeFromQueue(item.clientItemId)}>×</button> : null}</div>
                {(item.status === 'looking_up' || item.status === 'queued') ? <p className="bulk-add-queue-item__detail" aria-live="polite">{albumLookupStatusLabel(item.status)}…</p> : <div className="bulk-add-review"><Field label="Title"><input value={item.draft.title} aria-invalid={!item.draft.title.trim()} onChange={event => updateDraft(item.clientItemId, { title: event.target.value })} /></Field><Field label="Format"><select value={item.draft.mediaFormat} onChange={event => updateDraft(item.clientItemId, { mediaFormat: event.target.value as typeof item.draft.mediaFormat })}>{FORMATS.map(format => <option key={format}>{format}</option>)}</select></Field><AlbumVocabularyPicker label="Artists" options={artists.data.items.map(artist => ({ id: artist.person_id, name: displayArtist(artist) }))} selectedIds={item.draft.artistIds} onChange={artistIds => updateDraft(item.clientItemId, { artistIds })} onAdd={() => void addCanonicalArtist(item.clientItemId)} /><AlbumVocabularyPicker label="Genres" options={genres.data.map(genre => ({ id: genre.genre_id, name: genre.name }))} selectedIds={item.draft.genreIds} onChange={genreIds => updateDraft(item.clientItemId, { genreIds })} onAdd={() => void addCanonicalGenre(item.clientItemId)} /><Field label="Label"><input value={item.draft.label} onChange={event => updateDraft(item.clientItemId, { label: event.target.value })} /></Field><Field label="Release date"><input value={item.draft.releaseDate} onChange={event => updateDraft(item.clientItemId, { releaseDate: event.target.value })} /></Field>
                    ...
                    {item.result?.catalog_state === 'ambiguous' ? <Alert variant="warning">This catalog match must be resolved outside Bulk Add before it can be saved.</Alert> : null}
                    {item.result?.catalog_state === 'unshelved' ? <Alert variant="warning">This album already exists without a crate or wishlist membership. {item.result.catalog_album_ids?.length === 1 ? <AppLink className="bulk-add-review__resolution-link" to={`/albums/${item.result.catalog_album_ids[0]}`}>Open the album to add it to a wishlist.</AppLink> : 'Resolve the matching catalog record outside Bulk Add.'}</Alert> : null}
            ...
            <div className="bulk-add-save"><Button type="button" mutating disabled={!importable.length || importer.isPending} onClick={() => void save()}>{importer.isPending ? 'Saving…' : `Save Crate (${importable.length})`}</Button></div>
